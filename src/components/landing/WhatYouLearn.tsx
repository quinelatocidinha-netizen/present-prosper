...
export default function WhatYouLearn() {
  return (
    <section className="relative py-24 md:py-32">
      ...
      <div className="absolute bottom-0 left-1/2 w-[500px] h-[500px] bg-primary/5 blur-[150px] rounded-full -translate-x-1/2 pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-5 md:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-4 md:gap-5">
          {topics.map(({ icon: Icon, title }, i) => (
            <ScrollReveal key={i}>
              <div className="group flex items-start gap-5 p-6 md:p-7 rounded-2xl border border-border/30 bg-card/30 hover:bg-card/60 hover:border-primary/20 transition-all duration-300">
                <div className="flex-shrink-0 flex items-center gap-4">
                  <span className="font-serif text-2xl font-bold text-primary/20 group-hover:text-primary/40 transition-colors min-w-[2rem] text-right">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <div className="w-10 h-10 rounded-xl bg-primary/10 group-hover:bg-primary/15 flex items-center justify-center transition-colors">
                    <Icon className="w-4 h-4 text-primary" />
                  </div>
                </div>
                <p className="font-sans text-sm md:text-base text-secondary-foreground leading-relaxed pt-2">
                  {title}
                </p>
              </div>
            </ScrollReveal>
